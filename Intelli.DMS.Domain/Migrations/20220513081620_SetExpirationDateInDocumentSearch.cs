...
{
    public partial class SetExpirationDateInDocumentSearch : Migration
    {
 ...
 ALTER View [dbo].[DocumentSearchView] As
							...
							,
							TRY_Cast((select string_agg( ISNULL(meta.FieldValue, ' '), ',') from dbo.BatchMeta meta where meta.BatchItemReference=bi.BatchItemReference and (Select DocumentClassFieldTypeId from DocumentClassFields where Id =meta.DocumentClassFieldId)=10) as bigint) AS ExpirationDate
